...
extract_text_from_local() and gets back a small dict envelope:

    {"text": str, "truncated": bool, "warning": str | None}
    {"error": str}
...
# Hard truncation cap on extracted text — about 15k tokens.
MAX_CHARS = 50_000
...
def extract_text_from_drive(file_id, mime_type):
...
def extract_text_from_local(file_path):
    ...
    return _extract_by_mime(data, mime)


def _extract_by_mime(data, mime_type):
    """Dispatch raw bytes to the right extractor based on mime type."""
    if mime_type == PDF_MIME:
        return _wrap_plaintext(_extract_pdf(data))
    if mime_type == DOCX_MIME:
        return _wrap_plaintext(_extract_docx(data))
    if mime_type == XLSX_MIME:
        return _wrap_plaintext(_extract_xlsx(data))
...
def _wrap_plaintext(text):
    """Apply truncation and scanned/empty warnings to extracted text."""
    if text is None:
    ...
    if len(text) > MAX_CHARS:
        original = len(text)
        text = text[:MAX_CHARS] + f"\n\n...[truncated from {original} to {MAX_CHARS} chars]"
        truncated = True
    ...
    return {"text": text, "truncated": truncated, "warning": warning}
