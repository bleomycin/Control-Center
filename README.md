...
| Docker | `/app/backups/` (named volume `blaine-backups`) | `BACKUP_DIR` env var |

### Docker Backup Volume

Docker Compose mounts a persistent `blaine-backups` volume at `/app/backups/`. Backups survive container rebuilds.
...
| Username | *(blank)* |
| Password | *(blank)* |
| From Email | `noreply@blaine.local` |
| Admin Email (recipient) | `admin@blaine.local` |
...
PDF sections include key-value info tables, related entity tables, and text blocks. Generic utility at `blaine/pdf_export.py`.
...
|--------|------------|----------|
| `blaine-data` | `/app/data/` | SQLite database |
| `blaine-media` | `/app/media/` | Uploaded files |
| `blaine-backups` | `/app/backups/` | Backup archives |
