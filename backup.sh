...
Environment Variables:
  RESTIC_REPOSITORY     Repository path (default: /opt/docker-backups/<app-dir-name>)
...
First-Time Setup:
  1. ./backup.sh install   (downloads restic + generates RESTIC_PASSWORD in .env)
  2. ./backup.sh init       (creates restic repository)
  3. ./backup.sh snapshot   (take first snapshot)
HELP
    exit 0
}
...
cmd_install() {
    ...
    # Auto-generate RESTIC_PASSWORD in .env if not already present
    local env_file="${APP_DIR}/.env"
    if [[ -f "$env_file" ]] && grep -q '^RESTIC_PASSWORD=' "$env_file"; then
        log "RESTIC_PASSWORD already exists in .env — not overwriting"
    else
        local generated_pw
        generated_pw="$(openssl rand -base64 32)"
        echo "" >> "$env_file"
        echo "# Restic backup password (auto-generated by backup.sh install)" >> "$env_file"
        echo "RESTIC_PASSWORD=\"${generated_pw}\"" >> "$env_file"
        success "Generated RESTIC_PASSWORD and added to .env"
        warn "IMPORTANT: Back up this password. Without it, snapshots are unrecoverable."
    fi
}
...
RESTIC_REPOSITORY="${RESTIC_REPOSITORY:-/opt/docker-backups/$(basename "$APP_DIR")}"
