...
- **Forms**: `TailwindFormMixin` in `blaine/forms.py` auto-applies dark-mode classes; forms load choices dynamically in `__init__`
...
- **Exports**: CSV via `blaine/export.py`; PDF via `blaine/pdf_export.py` (reportlab platypus) with section types: "info", "table", "text"
