...
- **Advanced Filtering**: All list pages use `<form id="filter-form">` wrapping all inputs. Sortable column headers with `sort`/`dir` query params and arrow indicators — sort headers target `#task-content` (tasks) and `#note-content` (notes) so the full content area including sort controls is re-rendered on each click, keeping asc/desc toggle URLs fresh. Notes sort toolbar + cards + pagination extracted into `_note_list_content.html` partial. Priority and status sort use `Case/When` annotations for logical order (critical→high→medium→low, not alphabetical). Date range `<input type="date">` filters. Multi-select status/type via checkbox groups with `getlist()`.
