...
| **tasks** | Task, FollowUp | Task system — deadlines, priorities, status tracking, follow-up/stale outreach workflows; bidirectional direction (personal/outbound/inbound); multi-stakeholder M2M with grouped dropdown; meeting task type with optional time and meeting-to-notes workflow |
...
- **Docker**: Single container runs Gunicorn (foreground, 2 workers, 30s timeout) + qcluster (background). `entrypoint.sh` handles migrate, collectstatic, createsuperuser, sample data loading. Named volumes for SQLite (`blaine-data`), media (`blaine-media`), and backups (`blaine-backups`)
...
- **Meeting Tasks**: `"meeting"` added to `TASK_TYPE_CHOICES`. `due_time` optional TimeField (null/blank) on Task — separate from `due_date` DateField (zero blast radius on existing code). `is_meeting` property returns `self.task_type == "meeting"`. `scheduled_datetime_str` property returns ISO datetime string when time set, date-only string otherwise. Form: `due_time` field with `type="time"` widget, `clean()` rejects time without date. Template JS shows/hides time input based on `task_type == "meeting"` (on change + on load). Detail page: "Meeting Time" label with blue time display for meetings, blue-bordered "Meeting Notes" section with prominent "+ Add Meeting Notes" button (pre-fills note creation with title, note_type=meeting, task, first stakeholder, date). Table rows: blue `[MTG]` indicator before title, time in due column. Calendar: `scheduled_datetime_str` for timed start, `allDay: false` for meetings with time, `[MTG]` prefix. Timeline: time appended to meeting task summaries. Dashboard deadlines: blue color for meetings (not yellow), time in title. CSV export: "Time" column. PDF export: time appended to due date row. Notifications: `[MEETING]` prefix on overdue meeting lines. Admin: `due_time` in `list_display`. Upcoming deadlines template: blue badge with "Meeting" label.
...
- Unit/integration tests (359 tests across all modules)
...
- Dashboard rework — asset/task/legal focus replacing cashflow-heavy layout. Liquidity alerts now dismissable via sessionStorage (`static/js/dismiss-alerts.js`). Net worth + cashflow consolidated from 7 cards (2 rows) to 4 cards (1 row): Net Worth, Total Assets, Total Liabilities, Monthly Net Flow. New Asset Overview panel (`_asset_summary.html`) shows property/investment/loan counts and totals with color-coded links. Primary 3-column row (Overdue Tasks | Active Legal | Asset Overview) above secondary 2-column row (Stale Follow-ups | Recent Activity). Removed 4-card cashflow detail row and projected inflow/outflow queries — that granularity lives on `/cashflow/`. View context adds `monthly_net_flow`, `property_count`/`property_value`, `investment_count`/`investment_value`, `active_loan_count`/`loan_balance`.
...
- Meeting task support — `"meeting"` task type with optional `due_time` TimeField (separate from `due_date` for zero blast radius). `is_meeting` and `scheduled_datetime_str` properties. Form shows/hides time input via JS when task_type is "meeting"; `clean()` validates time requires date. Detail page: "Meeting Time" label, blue time display, blue-bordered "Meeting Notes" section with "+ Add Meeting Notes" button pre-filling note creation (title, type, task, stakeholder, date). Table rows: `[MTG]` indicator + time. Calendar: timed events (`allDay: false`) with `[MTG]` prefix. Timeline/deadlines: time in summaries, blue meeting badges. CSV "Time" column, PDF time in due date. Notifications: `[MEETING]` prefix. 10 new tests (369 total).
