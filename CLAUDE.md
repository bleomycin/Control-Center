...
- **Advanced Filtering**: All list pages use `<form id="filter-form">` wrapping all inputs. Sortable column headers with `sort`/`dir` query params and arrow indicators — sort headers target `#task-content` (not `#task-table-body`) so the full table including headers is re-rendered on each click, keeping asc/desc toggle URLs fresh. Priority and status sort use `Case/When` annotations for logical order (critical→high→medium→low, not alphabetical). Date range `<input type="date">` filters. Multi-select status/type via checkbox groups with `getlist()`.
